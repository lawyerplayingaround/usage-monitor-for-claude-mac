...
- **Viewport-deficit pad (`_layout_and_reveal`) - do not remove or "simplify" this on upstream merges.** On mixed-DPI multi-monitor setups, pywebview's `resize()` yields a WebView viewport (`window.innerHeight`) shorter than the requested window height - enough to clip the popup footer - so the window must be grown by the measured deficit while still transparent, before reveal. The correction is deliberately a bounded two-pass loop with a settle sleep (a tight loop oscillates: each resize re-rasterizes and shifts the measurement), bidirectional (a stale pad from another monitor shrinks back), clamped to 300 px, and remembered in `_learned_pad` for the session. Post-reveal height reports go through the `_HEIGHT_DEADBAND` guard so DPI jitter cannot start a resize feedback loop. This fix existed in the 1.15 fork, was silently lost in the v1.20.0 merge because it was undocumented, and shipped two broken releases before being restored - when merging upstream popup changes, re-verify `_layout_and_reveal`/`_apply_height`/`_set_content_height` survive
